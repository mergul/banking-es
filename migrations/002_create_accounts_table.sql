...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_account_projections_active_created
    ON account_projections (is_active, created_at DESC)
    WITH (fillfactor = 90);

-- Add index for balance range queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_account_projections_balance
    ON account_projections (balance)
    WITH (fillfactor = 90);

-- Add index for updated_at queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_account_projections_updated_at
