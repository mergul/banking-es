...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transaction_projections_recent
    ON transaction_projections (timestamp)
    WHERE timestamp > NOW() - INTERVAL '30 days'
    WITH (fillfactor = 90);

-- Add index for amount range queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transaction_projections_amount
    ON transaction_projections (amount)
    WITH (fillfactor = 90);

-- Add composite index for account type queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transaction_projections_account_type
