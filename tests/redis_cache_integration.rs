use banking_es::application::AccountService;
use banking_es::infrastructure::{
    AccountRepository, EventStore, EventStoreConfig, ProjectionStore,
    RealRedisClient, RedisClientTrait,
};
use banking_es::domain::AccountError; // For checking specific errors
...
async fn setup_test_environment() -> Result<TestContext, Box<dyn std::error::Error>> {
    ...
    let event_store = EventStore::new(db_pool.clone()).await?; // Assuming EventStore::new takes PgPool
    let projection_store = ProjectionStore::new(db_pool.clone());

    // Setup Redis Client
    let native_redis_client = NativeRedisClient::open(redis_url)?;
    let redis_client_trait: Arc<dyn RedisClientTrait> = RealRedisClient::new(native_redis_client.clone());
...
async fn get_account_current_version(account_id: Uuid, pool: &PgPool) -> Result<i64, sqlx::Error> {
    let row: Option<(i64,)> = sqlx::query_as("SELECT MAX(sequence_number) FROM events WHERE aggregate_id = $1")
        .bind(account_id)
        .fetch_optional(pool)
        .await?;
...
async fn get_dlq_items(redis_client: &Arc<dyn RedisClientTrait>) -> Result<Vec<String>, redis::RedisError> {
    let mut conn = redis_client.get_async_connection().await?;
    conn.lrange(TEST_DLQ_KEY, 0, -1).await
}

// Helper to clear DLQ
async fn clear_dlq(redis_client: &Arc<dyn RedisClientTrait>) -> Result<(), redis::RedisError> {
    let mut conn = redis_client.get_async_connection().await?;
    conn.del(TEST_DLQ_KEY).await
}

// Helper to get all items from a specific batching list
async fn get_batch_list_items(redis_client: &Arc<dyn RedisClientTrait>, account_id: Uuid) -> Result<Vec<String>, redis::RedisError> {
    let mut conn = redis_client.get_async_connection().await?;
    let key = format!("events_for_batching:{}", account_id); // Assuming prefix from repository.rs
...
async fn test_account_data_caching_and_invalidation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(ref client) = context.redis_conn_for_direct_checks {
        flush_redis_db(client).await.expect("Failed to flush Redis DB for test");
    }
    ...
    assert_eq!(account_v3.balance, initial_balance + deposit_amount, "Balance not updated after deposit and re-fetch");
...
async fn test_command_deduplication_integration() -> Result<(), Box<dyn std::error::Error>> {
    let context = setup_test_environment().await?;
    let redis_client_trait = RealRedisClient::new(context.redis_conn_for_direct_checks.clone().unwrap());

    flush_redis_db(&context.redis_conn_for_direct_checks.as_ref().unwrap()).await.expect("Failed to flush Redis DB for test");
    clear_dlq(&redis_client_trait).await.expect("Failed to clear DLQ before test");
    ...
    let account_id = context.account_service.create_account(format!("{}_Account",owner_name_base), initial_balance).await?;

    let deposit_amount = Decimal::new(10,0);

    // 1. First call to deposit_money (should succeed)
    let res1 = context.account_service.deposit_money(account_id, deposit_amount).await;
    assert!(res1.is_ok(), "First deposit call failed: {:?}", res1.err());

    // 2. Second call (immediate duplicate, should fail)
    let res2 = context.account_service.deposit_money(account_id, deposit_amount).await;
    assert!(res2.is_err(), "Second deposit call should have failed as duplicate");
    if let Err(AccountError::InfrastructureError(msg)) = res2 {
        assert!(msg.contains("Duplicate deposit command"), "Error message mismatch for duplicate: {}", msg);
    } else {
    ...
    if let Some(ref client) = context.redis_conn_for_direct_checks {
        ...
        let _ : () = redis::cmd("DEL").arg(&dedup_key).query_async(&mut conn).await?;
    ...
    let res3 = context.account_service.deposit_money(account_id, deposit_amount).await;
    assert!(res3.is_ok(), "Third deposit call failed after TTL/DEL: {:?}", res3.err());
...
async fn test_event_flusher_successful_batch_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(initial_version, 1, "Account creation event should result in version 1");
    ...
    assert_eq!(final_version, initial_version + 1, "Version should increment after deposit event");
    ...
    assert!(account_after_deposit_opt.is_some(), "Account not found after deposit");
    let account_after_deposit = account_after_deposit_opt.unwrap();
    assert_eq!(account_after_deposit.balance, initial_balance + deposit_amount, "Balance in account data incorrect after deposit");
    assert_eq!(account_after_deposit.version, final_version, "Version in account data incorrect after deposit");


    let batch_list = get_batch_list_items(&redis_client_trait, account_id).await?;
    assert!(batch_list.is_empty(), "Redis event batching list should be empty after successful flush");

    let dlq_items = get_dlq_items(&redis_client_trait).await?;
    assert!(dlq_items.is_empty(), "DLQ should be empty after successful processing. Found: {:?}", dlq_items);
...
async fn test_event_flusher_version_conflict_moves_to_dlq() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(version_after_create, 1, "Account should be version 1 after creation and flush.");
    ...
    let version_after_manual_insert = get_account_current_version(account_id, &context.db_pool).await?;
    assert_eq!(version_after_manual_insert, version_after_create + 1, "DB version not updated by manual insert.");
    ...
    let dlq_items = get_dlq_items(&redis_client_trait).await?;
    assert_eq!(dlq_items.len(), 1, "DLQ should contain one item after version conflict. Found: {:?}", dlq_items);
    ...
    let failed_batch_json = dlq_entry.get("failed_batch_item").expect("DLQ entry missing failed_batch_item");
    let failed_batch: banking_es::infrastructure::RedisEventBatchForTest = serde_json::from_value(failed_batch_json.clone())?;

    assert_eq!(failed_batch.expected_version, version_after_create, "DLQ'd batch has incorrect expected_version");
    assert_eq!(failed_batch.events.len(), 1, "DLQ'd batch should have one event"); // Assuming deposit creates one event

    // Verify no new events from the conflicting batch were persisted
    let current_db_version_after_flush_attempt = get_account_current_version(account_id, &context.db_pool).await?;
    assert_eq!(current_db_version_after_flush_attempt, version_after_manual_insert, "DB version should not change after failed flush");
...
mod banking_es {
    ...
    pub mod domain { // Assuming AccountEvent is here
        use rust_decimal::Decimal;
        use serde::{Deserialize, Serialize};
        use uuid::Uuid;
        ...
        pub enum AccountEvent {
            AccountCreated { account_id: Uuid, owner_name: String, initial_balance: Decimal },
            MoneyDeposited { account_id: Uuid, amount: Decimal, transaction_id: Uuid },
            MoneyWithdrawn { account_id: Uuid, amount: Decimal, transaction_id: Uuid },
            AccountClosed { account_id: Uuid, reason: String },
        }
    }
}
...
async fn test_event_flusher_multiple_batches_same_account() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create account
    let account_id = context.account_service.create_account(owner_name, initial_balance).await?;
    ...
    context.account_service.deposit_money(account_id, deposit1_amount).await?;
    ...
    context.account_service.deposit_money(account_id, deposit2_amount).await?;
    ...
    assert_eq!(final_version, version_after_create + 2, "Version should reflect both deposits.");
    ...
    assert_eq!(account.balance, initial_balance + deposit1_amount + deposit2_amount, "Final balance is incorrect.");
    assert_eq!(account.version, final_version, "Final account version in data is incorrect.");

    let batch_list = get_batch_list_items(&redis_client_trait, account_id).await?;
    assert!(batch_list.is_empty(), "Redis event batching list should be empty after flushing multiple batches. Found: {:?}", batch_list);

    let dlq_items = get_dlq_items(&redis_client_trait).await?;
    assert!(dlq_items.is_empty(), "DLQ should be empty after successful multi-batch processing. Found: {:?}", dlq_items);
